...
public class RepairRecipeRegistrar {
  final JavaPlugin javaPlugin;

  public RepairRecipeRegistrar(JavaPlugin javaPlugin) {
    this.javaPlugin = javaPlugin;
    ConfigurationSection section = javaPlugin.getConfig().getConfigurationSection("Repair");
    if (section != null && section.getBoolean("enabled")) {
      List<String> entries = section.getStringList("shape");
      ItemStack item = BlissItems.createRecipeItem();
      ShapedRecipe shapedRecipe = new ShapedRecipe(new NamespacedKey(javaPlugin, "repair_custom_crafting"), item);
