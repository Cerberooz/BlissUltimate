...
public class UpgraderRecipeRegistrar {
  final JavaPlugin javaPlugin;

  public UpgraderRecipeRegistrar(JavaPlugin javaPlugin) {
    this.javaPlugin = javaPlugin;
    ConfigurationSection section = javaPlugin.getConfig().getConfigurationSection("Upgrader");
    if (section != null && section.getBoolean("enabled")) {
      List<String> entries = section.getStringList("shape");
      ItemStack item = BlissItems.createUpgraderMenuItem();
      ShapedRecipe shapedRecipe =
          new ShapedRecipe(new NamespacedKey(javaPlugin, "upgrader_custom_crafting"), item);
